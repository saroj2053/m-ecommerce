...
router.get('/shipping-form', passportConf.isAuthenticated, function(req, res, next) {
	Cart.findOne({ owner: req.user._id }).populate('items.item').exec(function(err, foundCart) {
		if (err) return next(err);
...
router.post('/shipping-form', passportConf.isAuthenticated, function(req, res, next) {
	Cart.findOne({ owner: req.user._id }).populate('items.item').exec(function(err, foundCart) {
		if (err) return next(err);
...
router.get('/initiatePayment', passportConf.isAuthenticated, function(req, res, next) {
	Cart.findOne({ owner: req.user._id }).populate('items.item').exec(function(err, foundCart) {
		if (err) return next(err);
...
router.post('/', function(req, res, next) {
	...
	Product.find({ $text: { $search: q } }, { _id: 0, __v: 0 }, function(err, products) {
		if (err) {
			console.log(err);

		} else {
			res.render('main/product_searched', { products: products, category: category });
		}
	});
});
...
router.get('/product-checkout', function(req, res) {
	Cart.findOne({ owner: req.user._id }).populate('items.item').exec(function(errors, cartItems) {
		if (errors) {
			console.log(errors)
		} else {
